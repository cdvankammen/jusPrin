...
function build_universal() {
    ...
    mkdir -p "$PROJECT_BUILD_DIR/JusPrin"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/JusPrin/JusPrin.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$PROJECT_DIR/build/arm64/JusPrin/JusPrin.app" "$UNIVERSAL_APP"

    # Get the binary path inside the .app bundle
    BINARY_PATH="Contents/MacOS/JusPrin"

    # Create universal binary using lipo
    lipo -create \
        "$PROJECT_DIR/build/x86_64/JusPrin/JusPrin.app/$BINARY_PATH" \
        "$PROJECT_DIR/build/arm64/JusPrin/JusPrin.app/$BINARY_PATH" \
        -output "$UNIVERSAL_APP/$BINARY_PATH"

    echo "Universal binary created at $UNIVERSAL_APP"

    # Create universal binary for profile validator if it exists
    if [ -f "$PROJECT_DIR/build/arm64/JusPrin/OrcaSlicer_profile_validator.app/Contents/MacOS/OrcaSlicer_profile_validator" ] && \
       [ -f "$PROJECT_DIR/build/x86_64/JusPrin/OrcaSlicer_profile_validator.app/Contents/MacOS/OrcaSlicer_profile_validator" ]; then
        echo "Creating universal binary for OrcaSlicer_profile_validator..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/JusPrin/OrcaSlicer_profile_validator.app"
        rm -rf "$UNIVERSAL_VALIDATOR_APP"
        cp -R "$PROJECT_DIR/build/arm64/JusPrin/OrcaSlicer_profile_validator.app" "$UNIVERSAL_VALIDATOR_APP"
        ...
        # Create universal binary using lipo
        lipo -create \
            "$PROJECT_DIR/build/x86_64/JusPrin/OrcaSlicer_profile_validator.app/$VALIDATOR_BINARY_PATH" \
            "$PROJECT_DIR/build/arm64/JusPrin/OrcaSlicer_profile_validator.app/$VALIDATOR_BINARY_PATH" \
