...
if [[ ! -f "./scripts/flatpak/io.obico.JusPrin.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/io.obico.JusPrin.yml${NC}"
    exit 1
fi
...
if ! flatpak-builder \
    "${BUILDER_ARGS[@]}" \
    "$BUILD_DIR/build-dir" \
    scripts/flatpak/io.obico.JusPrin.yml; then
...
echo -e "flatpak run io.obico.JusPrin"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user io.obico.JusPrin"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
